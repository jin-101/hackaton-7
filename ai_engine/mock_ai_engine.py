    ...
    def analyze_strategy(self, issue_text: str, context: dict) -> dict:
        ...
        if any(k in issue_text for k in keywords_surge):
            desc = (
                ...
                f' 현재 L/F {lf}% 기준 상위 클래스(C·Y) 인벤토리 즉시 회수를 권고합니다.'
            ...
            desc = (
                ...
                f' M·V 클래스 공급 확대 및 운임 {round((1 - 0.90) * 100)}% 인하를 통해 가격 민감 고객층 유입을 유도하십시오.'
                f' 단, 프레스티지(C) 클래스는 현행 운임을 유지하여 수익성 하한선을 방어하시기 바랍니다.'
            )
            price_factor = 0.90
        else:
            if lf >= 75:
                trend = f"예약 페이스가 목표치를 상회(L/F {lf}%)하고 있으며, 상위 클래스 잔여 인벤토리 축소 추세가 확인됩니다."
                action = "C·Y 클래스 운임을 소폭 인상하고, M 클래스 오픈 좌석을 선제적으로 축소하여 단가 방어를 권고합니다."
                price_factor = 1.08
            elif lf <= 55:
                trend = f"예약 유입 속도가 계절적 기준치 대비 저조(L/F {lf}%)하며, 출발 임박 할인 수요 의존도가 높아질 위험이 있습니다."
                action = "V 클래스 운임을 전략적으로 인하하여 조기 예약을 촉진하고, 경쟁사 대비 가격 포지셔닝을 재점검하십시오."
                price_factor = 0.95
            else:
                ...
                price_factor = 1.03
            desc = (
                f'[{flight_ref}] "{issue_text}" 분석 완료 — {trend}'
                f' {action}'
            )
