...
- **결과**: `MockAiEngine` (오프라인), `ClaudeAiEngine` (프로덕션) 동일 인터페이스
...
### ADR-005: Property-Based Testing 도입
...
│  │  │  │  ClaudeAiEngine ──► 외부 Anthropic API          │  │  │  │
│  │  │  │  MockAiEngine   ──► 로컬 Mock (Fallback)        │  │  │  │
│  │  │  └─────────────────────────────────────────────────┘  │  │  │
│  │  └───────────────────────────────────────────────────────┘  │  │
│  │                                                             │  │
...
┌──────────────────────────────────────────────────────────┐
│                  View Layer (Pages)                      │
│  Dashboard │ FareManagement │ AiRec │ Competitor │ ...  │
├──────────────────────────────────────────────────────────┤
│                  State Layer (Zustand)                   │
│  dashboardStore │ fareStore │ aiRecStore │ ...           │
├──────────────────────────────────────────────────────────┤
│                  API Layer (ApiClient)                   │
...
│   │   └── FlightList
│   └── Step2: FareEditor
│       ├── ClassEditCard (4개 Tier)
│       ├── SeatMap (B737-900 배치도)
...
AbstractSimulationEngine (인터페이스)
      ...
      └── MockSimulationEngine
          ├── IATA 단거리 아시아 탄력성 계수
          │   (프레스티지: -0.6, 정상: -1.2, 할인: -1.5, 특가: -2.0)
          ├── 유가·환율·가격 변동 복합 계산
          └── 의사난수 시드 고정 (재현 가능)
```
