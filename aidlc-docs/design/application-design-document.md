...
- **운항 노선**: 국내선 8개 노선 (GMP-CJU, GMP-PUS, GMP-TAE, GMP-KWJ, ICN-CJU, ICN-PUS, GMP-KPO, GMP-RSU)
- **기종**: 3종 (B737-900ER 200석, B737-800 158석, A220-300 130석)
...
| FR-07 | 보고서 자동 생성 (PDF) | Report, ReportService |
...
│  ├─ Dashboard  ├─ flightsStore     ├─ ApiClient         │
│  ├─ Fares      ├─ dashboardStore   └─ Types (index.ts)  │
│  ├─ Competitor ├─ fareStore                             │
│  ├─ Simulator  ├─ aiRecStore                            │
│  └─ Report     ├─ simulationStore                       │
│                └─ reportStore                           │
└─────────────────────────────────────────────────────────┘
                         │ HTTPS / REST JSON
┌──────────────────────────────────────────────────────────┐
...
| 컴포넌트 | 의존 대상 |
|---|---|
| Dashboard | flightsStore (실시간 연동), dashboardStore → ApiClient → /api/dashboard |
| FareManagement | flightsStore, fareStore, aiRecommendationStore → ApiClient → /api/fares, /api/rm |
| AiRecommendations | FareManagement 내부 임베드 (별도 페이지 없음) |
...
- **기능**: 항공편별 좌석 클래스 운임·공급석 조회·수정 + AI 추천 적용
- **Step 1**: 날짜·노선 선택 → 항공편 목록 조회 (AI 추천 레이블 표시)
- **Step 2**: 항공편 선택 → 클래스별 운임 편집 + AI 추천 상세보기 + 기내 좌석 배치도 + 인벤토리 로그
- **특징**: URL 라우팅 지원 (`/fares/KE1201`) — 새로고침 시 상세 페이지 유지
- **날짜별 상태 격리**: `appliedFlights`, `confirmedClasses` 키에 날짜 prefix 포함(`"${date}:${flightId}"`) — 날짜 변경 시 해당 날짜 상태만 표시, 되돌아오면 이전 상태 복원. 새로고침 시에만 전체 초기화
- **date 캐시**: `routeDateCache` (useRef Map, key=`"route:date"`) — 날짜 이동 시 서버 재요청 없이 캐시 반환
- **인벤토리 상태별 편집 제약**:
  - Open: 가격·좌석 수 모두 수정 가능, 상태 토글 가능
  - Closed: 가격 수정 불가(잠금), 좌석 수 수정 가능, 상태 토글 가능
  - Sold Out: 가격 수정 불가(잠금), 좌석 수 수정 가능(증가 허용), 상태 토글 불가
- **인벤토리 확정 흐름**: Step 2 확정 버튼 클릭 → 전 클래스에 대해 `PUT /api/fares/{flightId}` 순차 호출 → AI 추천가(aiPrice)와 현재가(price)가 다른 클래스는 aiPrice로 저장 → 성공 시 로컬 상태 동기화 + confirmedClasses/appliedFlights 갱신
- **Store**: `flightsStore` (대시보드 연동), `fareStore`, `aiRecommendationStore`
...
| Store | 상태 | 주요 액션 |
|---|---|---|
| flightsStore | flightsByRoute (route → DashboardFlight[]) | setFlightsForRoute, getFlightsForRoute, refreshAllRoutes |
...
**flightsStore 역할**: 운임관리(FareManagement)에서 LF·운임 변경 시 `setFlightsForRoute`로 저장 → Dashboard가 `flightsByRoute`에서 읽어 KPI 실시간 갱신. 노선 key만 관리하며 날짜 구분 없음 (날짜별 격리는 FareManagement 로컬 `routeDateCache`로 처리).
...
**탭 렌더링**: 모든 탭은 App.tsx에서 항상 마운트 유지 (`className={active ? "" : "hidden"}`). 탭 전환 시 언마운트 없음 — 내부 상태(선택된 항공편, 적용 이력 등) 보존.
...
- **탄력성 계수**: C(프레스티지) -0.45, Y(일반 정상) -0.95, M(할인) -1.35, V(특가) -1.75

#### CompetitorService
- **책임**: 경쟁사 가격 데이터 조회 및 자사 대비 비교

#### ReportService
- **책임**: 수익 보고서 데이터 집계 + pdf 생성
...
**기종별 좌석 배분** (seed_data.py AIRCRAFT_CONFIG 기준)
...
| BR-10 | Sold Out 등급은 가격 수정 불가, 좌석 수만 수정 가능 | UI 레벨 | FareManagement (priceLocked) |
| BR-11 | 등급별 좌석 수 합계는 항공편 전체 좌석 수 초과 불가 — 초과 시 에러 차단 | 저장 전 | aiReallocateSeats (eligibleMinSum 검증) |
...
| ID | 규칙 |
|---|---|
| BR-12 | 인벤토리 확정 버튼 클릭 시 AI 추천 운임(aiPrice)이 현재가와 다른 클래스는 aiPrice로 저장, 동일하거나 추천 없으면 현재가 유지 |
| BR-13 | 확정 성공 후 로컬 flights 상태도 price=aiPrice로 즉시 동기화 (UI 재렌더링) |
| BR-14 | 확정 버튼은 AI 추천 유무와 무관하게 Step 2 진입 시 항상 표시 |
